...
func Welling(environment *Location) {
  fmt.Printf(" ┌─ DEBUG [Welling]: reading bunch of positive power states - start.\n ├─ ")
  // campfires
  ...
  // elementalTree
  powerCore1 := primitives.Stream{Element: primitives.RNDElem(), Creation: math.Abs(primitives.SRNF()), Destruction: math.Abs(primitives.SRNF()), Alteration: math.Abs(primitives.SRNF())}
  powerCore2 := primitives.Stream{Element: primitives.RNDElem(), Creation: math.Abs(primitives.SRNF()), Destruction: math.Abs(primitives.SRNF()), Alteration: math.Abs(primitives.SRNF())}
  ...
  fmt.Printf("DEBUG [Welling]: power core is randomized for emitting %s and %s energy...\n", primitives.ES(powerCore1.Element), primitives.ES(powerCore2.Element))
  // windySpaces
  ...
  *&environment.Wells = buffer
  fmt.Printf(" └─ DEBUG [Welling]: reading bucnh of positive power states - done.\n")
}
